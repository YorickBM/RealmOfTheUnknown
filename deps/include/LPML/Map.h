...
namespace LowPolyMeshLibrary {
	...
	public:
		...
		void clearElements() //optimize //maybe make zero map
		{
		...
		vector<vector<float> > getElements() { return elements; }
		float getElement(int x, int y) { return elements[y][x]; }

		void setElement(int x, int y, int val) { elements[y][x] = val; }
		void loadElements(vector<vector<float> > input) { elements = input; } //maybe also do new equal to and new clear? 
		...
		void generate(int size, float height, bool x) //size is just x but make x better first, sidelength
		{
			srand(time(NULL));
		...
		void generate(int size, int roughness) //size is just x but make x better first, sidelength
		{
			srand(time(NULL));
			int squaresize = (size - 1) / 2;
		...
		void generate(int size, float roughness) //size is just x but make x better first, sidelength
		{
			srand(time(NULL));
			int squaresize = (size - 1) / 2;
		...
		void generate(int size, int roughness, int min, int max) //size is just x but make x better first, sidelength
		{
			srand(time(NULL));
		...
		void generate(int size, float mag, float roughness) //size is just x but make x better first, sidelength
		{
			srand(time(NULL));
			int squaresize = (size - 1) / 2;
