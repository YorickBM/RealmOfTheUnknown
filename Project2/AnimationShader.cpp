...
void AnimationShader::AddComponent(std::string filename, GLenum type)
{
...
void AnimationShader::Use()
{
	glUseProgram(identifier);
}

void AnimationShader::SetMat4(const std::string & name, const glm::mat4 & mat) const
{
	glUniformMatrix4fv(glGetUniformLocation(identifier, name.c_str()), 1, GL_FALSE, &mat[0][0]);
}

GLuint & AnimationShader::ID()
{
	return identifier;
}
