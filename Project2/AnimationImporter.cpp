...
void ProcessVertices(AnimationMesh* mesh, aiMesh* ai_mesh) {
...
void ProcessFaces(AnimationMesh* mesh, aiMesh* ai_mesh) {
...
void ProcessBones(AnimatedModel* model, AnimationMesh* mesh, aiMesh* ai_mesh) {
	...
	for (unsigned int i = 0; i < ai_mesh->mNumBones; i++)
	{
		...
		Bone* bn = new Bone{ ai_bone->mName.C_Str(), AIToGLMMat4(ai_bone->mOffsetMatrix) };
		std::shared_ptr<Bone> bone{ bn };
...
AnimationMesh* ProcessMesh(AnimatedModel* model, aiMesh* ai_mesh) {
...
AnimationNode* ExploreHeirarchy(AnimatedModel* model, aiNode* ai_node, const aiScene* ai_scene, glm::mat4 parent_transform) {
	AnimationNode* node = new AnimationNode(ai_node->mName.C_Str());
	...
	node->transform = relative_transform;
...
AnimatedModel* AnimationImporter::LoadAnimatedFile(std::string filename)
{
	Assimp::Importer importer;
	const aiScene* ai_scene = importer.ReadFile(filename, aiProcess_Triangulate | aiProcess_FlipUVs);

	if (ai_scene == NULL) {
		std::cout << "Error on loading Animated Model" << std::endl;
