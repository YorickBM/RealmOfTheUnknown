...
void GenBufferHierarchy(AnimationModel& model, AnimationNode& node) {
	node.ForEachMesh([&](AnimationMesh& mesh) {
	...
	node.ForEachChild([&](AnimationNode& child) {
		GenBufferHierarchy(model, child);
	});
}

void RenderHierarchy(AnimatedModel& model, AnimationNode& node) {
	node.ForEachMesh([&](AnimationMesh& mesh) {
...
void AnimationRenderer::Render(AnimatedModel& model)
{
	model.GetShader().Use();
	if (!model.IsBuffered()) {
