...
quat Bone::calcInterpolatedRotation(float time)
{
	...
	if (nodeAnim->mNumRotationKeys == 1) //if amount of keyframes == 1 we dont have to interpolate anything
	{
	...
	out = slerp(r1, r2, factor); //here we use another glm function to mix the quternion values dependent on factor
